#!/bin/bash
# Marketplace Bulk Editor - Interactive Setup Script (Fedora)
# This script guides you through setting up the full Docker backend
# Platform: Fedora/RHEL/CentOS (firewalld)
# Future platforms: Ubuntu (ufw), macOS, Windows (PowerShell)
# Future features: OCR processing capabilities
set -e
...
echo -e "${GREEN}"
echo "╔═══════════════════════════════════════════════════════════╗"
echo "║      Marketplace Bulk Editor - Full Setup Script          ║"
echo "║   PostgreSQL + Redis + JWT Auth + Backend API             ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${NC}"

# Step 1: Check Docker
log_step "Step 1/7: Checking Docker installation..."
...
log_step "Step 3/7: Checking repository..."
if [ -f "docker-compose.yml" ] && [ -f "backend/app.py" ]; then
    log_ok "Already in marketplace-bulk-editor directory"
    REPO_DIR="."
elif [ -d "marketplace-bulk-editor" ]; then
    log_ok "Found existing marketplace-bulk-editor directory"
    cd marketplace-bulk-editor
    REPO_DIR="marketplace-bulk-editor"
else
...
log_step "Step 4/7: Starting Docker containers (this may take 2-5 minutes on first run)..."
echo "    Building backend with PostgreSQL + Redis..."
...
log_step "Step 5/7: Waiting for backend to initialize..."
echo "    (Backend initializes database schema on first run)"
MAX_WAIT=120
WAIT_COUNT=0
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    if curl -s http://localhost:5000/ | grep -q '"status":"ok"'; then
        echo ""
...
if [ $WAIT_COUNT -ge $MAX_WAIT ]; then
    ...
    echo "Check logs with: docker logs marketplace-backend"
    exit 1
fi
...
if command -v ufw &> /dev/null; then
    ...
    if echo "$UFW_STATUS" | grep -q "active"; then
        ...
        echo "    ports 5173 (frontend) and 5000 (backend) must be opened."
        echo ""
        read -p "    Open firewall ports now? [Y/n] " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
            sudo ufw allow 5173/tcp comment "Marketplace Editor frontend"
            sudo ufw allow 5000/tcp comment "Marketplace Editor backend"
            log_ok "Firewall ports opened (5173, 5000)"
            FIREWALL_CONFIGURED=true
        else
            echo ""
            log_warn "Skipped. To enable network access later, run:"
            echo -e "    ${BLUE}sudo ufw allow 5173/tcp && sudo ufw allow 5000/tcp${NC}"
        fi
    else
...
# Check for firewalld (Fedora/RHEL/CentOS)
    ...
    if systemctl is-active --quiet firewalld; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
            ...
            echo ""
            log_warn "Skipped. To enable network access later, run:"
            echo -e "    ${BLUE}sudo firewall-cmd --add-port=5173/tcp --add-port=5000/tcp --permanent${NC}"
...
# Check for iptables directly
    ...
    if sudo iptables -L INPUT -n 2>/dev/null | grep -qE "(DROP|REJECT)"; then
        ...
        echo -e "    ${BLUE}sudo iptables -I INPUT -p tcp --dport 5000 -j ACCEPT${NC}"
    else
